...
class Dataset(object):
    def __init__(self, path, sep=',', session_key='SessionID', item_key='ItemId', time_key='timestamp', n_sample=-1, itemmap=None, itemstamp=None, time_sort=False):
        ...
        self.df = self.df.rename(columns={'new_sessionId':'SessionID','new_trackId':'ItemId'})
        self.df['item_idx'] = self.df.ItemId
...
class DataLoader():
    def __init__(self, dataset, item_mapper, audio_dir, batch_size=10):
        """
    ...
    def load_audio(self, batch_data, target):

        data_code = [] 
        for input_data in batch_data.cpu().detach().numpy():
            item_audio = self.item_mapper.loc[self.item_mapper.new_trackId == input_data].filename.values[0]
            audio = np.load(self.audio_dir+"npys/"+item_audio+".npy", allow_pickle=True)
            j = np.random.randint(9, size=1)[0]
            data_code.append(audio[j*(59049):(j+1)*59049])
           
        x = torch.tensor(data_code).type(torch.FloatTensor).view(self.batch_size,1,-1)
    ...
    def __iter__(self):
        ...
        while not finished:
            ...
            for i in range(minlen - 1):
                ...
                try:
                    x = self.load_audio(input, target)
